...
class BaseModelHttp {
		...
		const DAGGER_HTTP_REDO = 5; // 访问失败后的重试次数, 默认0次为不重试。
	 ...
	 *
	 **/
	public function curlCheck($state)
	{
		static $test = array(
			CURLE_OK => "CURLE_OK",
			...
			CURLE_FTP_WEIRD_USER_REPLY => "CURLE_FTP_WEIRD_PASS_REPLY",
			...
			CURLE_UNKNOWN_TELNET_OPTION => "CURLE_UNKNOW_TELNET_OPTION" , 
		...
		if(array_key_exists($state , $test)){
			...
			echo "the state doesn't exist ,please add from http://php.net/manual/en/curl.constants.php \n<br/>";
		}
